...
class produksi {
    ...
    function LihatPesanan() {
        $sqlLihatPesanan = mysqli_query($this->conn,"SELECT pemesanan.id_pesanan, pemesanan.nama_pemesan, pemesanan.proses,
    ...
    function findPesananById($id) {
        $sqlProduksi = mysqli_query($this->conn,"SELECT * FROM pemesanan WHERE id_pesanan = '$id'");
    ...
    function ProduksiTambah() {
        $sqlProduksiTambah = mysqli_query($this->conn,"INSERT INTO produksi VALUES('$this->id_produksi', '$this->id_pesanan', '$this->id_barang', '$this->jumlah_produksi','$this->lead_time')");
        // ini untuk set proses ke 1
        $updateProsesPesanan = mysqli_query($this->conn,"UPDATE `bullwhip`.`pemesanan` SET `proses` = '1' WHERE `pemesanan`.`id_pesanan` = '$this->id_pesanan';");
    }

    function DaftarProduksi() {
        $sqlDaftarProduksi = mysqli_query($this->conn,"SELECT
